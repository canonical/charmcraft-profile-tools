This repo contains the latest [Kubernetes](kubernetes) and [machine](machine) charms that `charmcraft init` generates, using Charmcraft from `main`. This repo also provides tools for maintaining the corresponding templates in the Charmcraft source.

The tools are primarily intended to be used by the Charm Tech team at Canonical. Here's the end-to-end maintenance process.
...
> [!IMPORTANT]
> Don't edit the `uv.lock.j2` files. The tools run `uv lock` in the generated charms and update the `uv.lock.j2` files accordingly. When you run the tools, you can pass `uv lock` options if needed.
...
3. Run `just kubernetes` and `just machine`. If `just` isn't available on your system, use `uvx --from rust-just just` instead.
...
To generate a charm that should pass integration tests, run `just kubernetes-extra` in your charmcraft-profile-tools clone. This command creates a [copy of the Kubernetes charm](kubernetes-extra) and replaces its placeholders by real configuration/code.
...
Commit changes in your charmcraft-profile-tools clone, then push your branch and open a PR. For example, see [charmcraft-profile-tools#53](ttps://github.com/canonical/charmcraft-profile-tools/pull/53).
...
1. After your charmcraft-profile-tools PR has passed all automated checks, commit changes in your Charmcraft clone. Use a conventional commit type **for each commit**. For example, `feat(templates):` or `chore(templates):`.
...
1. Go to the [Charmcraft main branch](https://github.com/canonical/charmcraft/tree/main) and grab the hash of the latest commit that updates the profiles (which is probably the commit coming from your merged PR).
